import React from "react";

function InsuranceDocumentsIcon() {
  return (
    <svg
      width="24"
      height="24"
      viewBox="0 0 24 24"
      fill="none"
      xmlns="http://www.w3.org/2000/svg"
    >
      <path
        d="M9.71503 12C10.866 12 11.715 11.151 11.715 10C11.715 8.849 10.866 8 9.71503 8C8.56403 8 7.71503 8.849 7.71503 10C7.71503 11.151 8.56303 12 9.71503 12Z"
        fill="#003B4A"
      />
      <path
        d="M20 4H4C2.897 4 2 4.841 2 5.875V18.125C2 19.159 2.897 20 4 20H20C21.103 20 22 19.159 22 18.125V5.875C22 4.841 21.103 4 20 4ZM20 18L4 17.989V6L20 6.011V18Z"
        fill="#003B4A"
      />
      <path
        d="M14 9H18V11H14V9ZM15 13H18V15H15V13ZM13.43 15.536C13.43 14.162 11.754 12.75 9.715 12.75C7.676 12.75 6 14.162 6 15.536V16H13.43V15.536Z"
        fill="#003B4A"
      />
    </svg>
  );
}

export default InsuranceDocumentsIcon;
