...
describe("App", () => {
    ...
    it("Set value to all field", async () => {
      ...
      expect(container.getByLabelText("Password unhide icon")).toBeInTheDocument();
    });
    ...
    it("Set Invalid value to all field and see error message", async () => {
      ...
      const saveButton = container.getByRole("button", { name: "scheduleAppoinment" });
      ...
      expect(
        container.getByText("Incorrect email format")
      ).toBeInTheDocument();
      ...
      await waitFor(() =>
        container.getByText("Incorrect First Name format")
      );
  ...
  it("Set mandatory field empty and see error message", async () => {
    ...
    await waitFor(() =>
      container.getAllByText("thisFieldRequired")
    );

  expect(
    container.getAllByText("thisFieldRequired")[0]
  ).toBeInTheDocument();
