...
describe("StyledInput", () => {
  it("renders input components with type text", async () => {
    ...
    const { container } = render(<PasswordValidator isShowValidation={true} validator={passwordValidator} password="password" />);
    expect(await screen.getByRole('heading', { name: 'Contain at least 3 our of 4 types'})).toBeVisible()
    expect(container).toMatchSnapshot();
  });

  it("renders input components with type text", async () => {
      const passwordValidator = [
        { label: 'Password length should range from 8 to 20 characters', validate: !RegExp(/^[^-\s]{8,20}$/) },
    ]
    const { container } = render(<PasswordValidator isShowValidation={true} validator={passwordValidator} password="password" />);
    expect(container.textContent).toBe("Password length should range from 8 to 20 characters");
  });

    it("renders input components with type text", async () => {
      const passwordValidator = [
        { label: 'Password should contain at least one numerical character (0-9)', validate: new RegExp('[0-9]') },
    ]
    const { container } = render(<PasswordValidator isShowValidation={true} validator={passwordValidator} password="password" />);
    // const input = container.getElementsByClassName("custom-input");
    // expect(input).toBeTruthy();
    expect(container.textContent).toBe("Password should contain at least one numerical character (0-9)");
