...
describe("Render change password", () => {
  ...
  test("confirm password not match", async () => {
    ...
    await waitFor(() => container.getByTestId("update-btn"));
  ...
  test("password same with current password", async () => {
    mock.onPost(`/ecp/patient/settings/validatePassword`).reply(200, {
      responseCode: 2000,
    ...
    await renderUpdatePasswordPage();
    const newPassField = container.container.querySelector("#new-password");
    fireEvent.change(newPassField, { target: { value: "Admin@1" } });
    const confPassField =
      container.container.querySelector("#confirm-password");
    fireEvent.change(confPassField, { target: { value: "Admin@1" } });
  ...
  //     await waitFor(() => container.getByTestId("update-btn"));
  //     fireEvent.click(container.getByTestId("update-btn"));
  //     await waitFor(() => container.getByTestId("custom-modal-close-btn"));
