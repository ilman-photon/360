import { act, fireEvent, getAllByTestId, render, waitFor, within } from "@testing-library/react";
import "@testing-library/jest-dom";
import AccountRecovery from "../../../src/pages/patient/admin/account-recovery";
import { renderWithProviders } from "../utils/test-util";
import { mockResponse } from "../../../src/pages/api/dummy/account-recovery/getPatientAccount/[keyword]";
import store from "../../../src/store/store";
import { Provider } from "react-redux";
import { mockSecurityQuestions } from "../../../src/pages/api/dummy/account-recovery/onViewSecurityQuestions/[patientId]";
import { setStatus } from "../../../src/store/accountRecovery";
...
let container;
export async function searchSomethingAndShowResult() {
  // global.fetch = () => {
  //   return new Promise((resolve) =>
  //     resolve({
  //       json: () => {
  //         return mockGetPatientAccountResponse;
  //       }
  //     })
  //   );
  // };
  ...
  const searchResultTitle = await waitFor(() => container.getAllByText(/Results found using your search criteria/i))
  expect(searchResultTitle[0]).toBeInTheDocument()
}

export async function searchMoreAccountRecoveryMenu(index) {
  const menus = await waitFor(() => container.getAllByTestId("table-cell-account-recovery-more-menu-btn"))
  expect(menus[index]).toBeInTheDocument()
  fireEvent.click(menus[index])
}

export async function clickActivateAccount() {
  global.fetch = () => {
    return new Promise((resolve) =>
      resolve({
        json: () => {
  ...
  const unlockBtn = await waitFor(() => container.getAllByTestId("menu-more-activate-account"))
  expect(unlockBtn[0]).toBeInTheDocument()
  fireEvent.click(unlockBtn[0])
}

async function clickUnlockAccount() {
  const unlockBtn = await waitFor(() => container.getAllByTestId("menu-more-unlock-account"))
  expect(unlockBtn[0]).toBeInTheDocument()
  fireEvent.click(unlockBtn[0])
  // global.fetch = () => {
  //   return new Promise((resolve) =>
  ...
  const mockSuccessUnlock = {
    responseCode: 3000,
    responseType: "Account is UnLocked Successfully",
  };
  const patientId = "8ad30e6a-db45-4147-b108-0d8a4c810782"
  mock
    .onGet(`/ecp/accountRecovery/unlockAccountByAdmin/${patientId}`)
    .reply(200, mockSuccessUnlock);
}

async function clickPasswordReset() {
  const unlockBtn = await waitFor(() => container.getAllByTestId("menu-more-send-password-reset"))
  expect(unlockBtn[0]).toBeInTheDocument()
  fireEvent.click(unlockBtn[0])
  // global.fetch = () => {
  //   return new Promise((resolve) =>
  ...
  //         return {
  //           success: true,
  //           message: `Password reset link sent to Adam successfully`,
  //         };
  //       }
  //     })
  //   );
  ...
  const mockSendPasswordReset = {
    ...
    email: "existi@email.com",
    token: 7647,
  };
  mock
    .onPost(`/ecp/accountRecovery/sendPasswordResetLinkToUser`)
    .reply(200, mockSendPasswordReset);
}

export async function clickShareUsername() {
  const unlockBtn = await waitFor(() => container.getAllByTestId("menu-more-share-username"))
  expect(unlockBtn[0]).toBeInTheDocument()
  fireEvent.click(unlockBtn[0])
  // global.fetch = () => {
  //   return new Promise((resolve) =>
  ...
  //         return {
  //           success: true,
  //           message: `Username shared to Adam successfully`,
  //         };
  //       }
  //     })
  //   );
  ...
  const mockShareUsername = {
    responseCode: 3000,
    responseType: "success",
  };
  mock
    .onPost(`/ecp/accountRecovery/shareUserNameByAdmin`)
    .reply(200, mockShareUsername);
}
...
describe("AccountRecoveryPage", () => {
  ...
  it("search something on the search bar and show search result", ()=> {
    searchSomethingAndShowResult()
  })

  it("shows more account-recovery menu", async () => {
    await searchSomethingAndShowResult()
    await searchMoreAccountRecoveryMenu(0)
  })

  it("click activate account and render modal and click confirm activate", async () => {
    await searchSomethingAndShowResult()
    await searchMoreAccountRecoveryMenu(2)
    await clickActivateAccount()
    const yesBtn = await waitFor(() => container.getByText("Yes"))
    fireEvent.click(yesBtn)

    // then close the success message
    const formMessageCloseBtn = await waitFor(() => container.getAllByTestId("table-form-message-close-btn"))
    fireEvent.click(formMessageCloseBtn[0])
  })

  it("click activate account and render modal and click cancel", async () => {
    await searchSomethingAndShowResult()
    await searchMoreAccountRecoveryMenu(2)
    await clickActivateAccount()
    const noBtn = await waitFor(() => container.getByText("No"))
    fireEvent.click(noBtn)
  })

  it("click activate account and render modal and click close modal button", async () => {
    await searchSomethingAndShowResult()
    await searchMoreAccountRecoveryMenu(2)
    await clickActivateAccount()
    const closeBtn = await waitFor(() => container.getByTestId("custom-modal-close-btn"))
    fireEvent.click(closeBtn)
  })

  it("click unlock account and render modal and click confirm unlock", async () => {
    await searchSomethingAndShowResult()
    await searchMoreAccountRecoveryMenu(0)
    await clickUnlockAccount()
    const yesBtn = await waitFor(() => container.getByText("Yes"))
    fireEvent.click(yesBtn)
  })
  
  it("click unlock account and render modal and click cancel", async () => {
    await searchSomethingAndShowResult()
    await searchMoreAccountRecoveryMenu(0)
    await clickUnlockAccount()
    const noBtn = await waitFor(() => container.getByText("No"))
  ...
  it("click send password reset link and render modal and click confirm send", async () => {
    await searchSomethingAndShowResult()
    await searchMoreAccountRecoveryMenu(0)
    await clickPasswordReset()
    const yesBtn = await waitFor(() => container.getByText("Send Password"))
    fireEvent.click(yesBtn)
  })
  
  it("click send password reset link and render modal and click cancel", async () => {
    await searchSomethingAndShowResult()
    await searchMoreAccountRecoveryMenu(0)
    await clickPasswordReset()
    const noBtn = await waitFor(() => container.getByText("Cancel"))
    fireEvent.click(noBtn)
  })

  it("click share username link and render modal and click confirm share", async () => {
    await searchSomethingAndShowResult()
    await searchMoreAccountRecoveryMenu(0)
    await clickShareUsername()
    const yesBtn = await waitFor(() => container.getByRole("button", {name: "Share Username"}))
    fireEvent.click(yesBtn)
  })

  it("click share username link and render modal and click cancel", async () => {
    await searchSomethingAndShowResult()
    await searchMoreAccountRecoveryMenu(0)
    await clickShareUsername()
    const noBtn = await waitFor(() => container.getByText("Cancel"))
  ...
  it("click view security question and render modal showing the questions", async () => {
    await searchSomethingAndShowResult()
    await searchMoreAccountRecoveryMenu(0)
    const securityQuestionBtn = await waitFor(() => container.getAllByTestId("menu-more-view-security-questions"))
    expect(securityQuestionBtn[0]).toBeInTheDocument()
    fireEvent.click(securityQuestionBtn[0])
    // global.fetch = () => {
    //   return new Promise((resolve) =>
    //     resolve(mockSecurityQuestions)
    //   );
    // };
    ...
    const securityQuestionTitle = await waitFor(() => container.getByTestId("modal-view-security-questions-title"))
    expect(securityQuestionTitle).toBeInTheDocument()
    expect(store.getState().accountRecovery.securityQuestions.length === 0).toBeTruthy()

    const closeBtn = await waitFor(() => container.getByText("Close"))
    fireEvent.click(closeBtn)
  })
})