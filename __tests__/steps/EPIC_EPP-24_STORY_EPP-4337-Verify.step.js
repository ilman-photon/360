...
const mockApi = () => {
	...
	mock
		.onGet(
			`/ecp/appointments/getDoctorDetails?pageSize=300&search.query=((firstName=eq=Robert)OR(lastName=eq=Robert)AND(providerDetails.specialization=eq=Glaucoma))`
		)
		.reply(200, mockDoctorSearch);
};
...
defineFeature(feature, (test) => {
	...
	test('EPIC_EPP-24_STORY_EPP-4337 - Verify User searches and selects a specialities to filter the doctors', ({ given, and, when, then }) => {
    	...
    	when(/^user click on dropdown of (.*) field$/, (arg0) => {
			fireEvent.change(container.container.querySelector("#speciality"), {
    ...
    test('EPIC_EPP-24_STORY_EPP-4337 - Verify User able to view filter result from specialities that they selected', ({ given, and, when, then }) => {
    	given('user launch Patient Portal url', () => {
			defaultValidation()
    	});
    ...
    test('EPIC_EPP-24_STORY_EPP-4337 - Verify User able to view filter button to open overlay', ({ given, and, when, then }) => {
    	given('user launch Patient Portal url', () => {
			defaultValidation()
    	});
