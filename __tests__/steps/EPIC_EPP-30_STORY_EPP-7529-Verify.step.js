...
const feature = loadFeature(
  "./__tests__/feature/Patient Portal/Sprint9/EPP-7529.feature"
);
...
defineFeature(feature, (test) => {
  ...
  async function seeSearchResult() {
    const searchResultTitle = await waitFor(() => container.getAllByText(/3 Results found using your search criteria/i))
    expect(searchResultTitle[0]).toBeInTheDocument()
  }

  async function adminSeeListPatientResult() {
    const tableContainer = await waitFor(() => container.getByTestId("table-container"))
    const tableSorts = await waitFor(() => container.getAllByTestId("table-header-sort"))
    expect(tableSorts[0]).toBeInTheDocument()
  }

  async function searchSomethingAndOpenMoreMenu() {
    const keyword = 'a'
    mock
      .onGet(`/ecp/accountRecovery/getPatientDetails/${keyword}`)
      .reply(200, mockGetPatientAccountAxios);
  ...
  async function clickThreeDots() {
    const moreMenuBtn = await waitFor(() => container.getAllByTestId("table-cell-account-recovery-more-menu-btn"))
    fireEvent.click(moreMenuBtn[0])
  }

  async function userClickShareUsernameCTA() {
    const shareUsernameBtn = await waitFor(() => container.getAllByTestId("menu-more-share-username"))
    expect(shareUsernameBtn[0]).toBeInTheDocument()
    fireEvent.click(shareUsernameBtn[0])
  }

  async function viewDialogConfirmationShareUsername() {
    const shareUsernameDialogTitle = await waitFor(() => container.getByText((/Are you sure you want to share username?/i)))
    expect(shareUsernameDialogTitle).toBeInTheDocument()
  }

  async function userViewShareUsernameBtn() {
    const shareButton = await waitFor(() => container.getByRole("button", {name: "Share Username"}))
    expect(shareButton).toBeInTheDocument()
  }

  async function userViewCancelBtn() {
    const cancelButton = await waitFor(() => container.getByRole("button", {name: "Cancel"}))
    expect(cancelButton).toBeInTheDocument()
  }

  async function userClickCancelBtn() {
    const cancelButton = await waitFor(() => container.getByRole("button", {name: "Cancel"}))
    fireEvent.click(cancelButton)
  }

  async function adminSeeSuccessShareUsernameMsg() {
    const formMessageCloseBtn = await waitFor(() => container.getAllByTestId("table-form-message-close-btn"))
    expect(formMessageCloseBtn[0]).toBeInTheDocument()
  }

  test('EPIC_EPP-30_STORY_EPP-7529- Verify Admin should be able to view Are you sure you want to share username? overlay', ({ given, and, when, then }) => {
    ...
    and('Admin should be able to view the list of patients based on search along with', (table) => {
      adminSeeListPatientResult()
    });
    ...
    when('admin click on Share Username sub menu', () => {
      userClickShareUsernameCTA()
    });

    then('admin should be able to view overlay with title written as Are you sure you want to share username?', () => {
      viewDialogConfirmationShareUsername()
    });
  });

  test('EPIC_EPP-30_STORY_EPP-7529- Verify Admin should be able to view the preferred mode of communication as selected by the patient', ({ given, and, when, then }) => {
    given('Admin get request from user (patient)', () => {
      defaultValidation()
    });
  ...
  test('EPIC_EPP-30_STORY_EPP-7529- Verify admin should not be able to view mode of communication option email if the email is not available in the patient information', ({ given, and, when, then }) => {
    given('Admin get request from user (patient)', () => {
      defaultValidation()
    });
  ...
  test('EPIC_EPP-30_STORY_EPP-7529- Verify admin should not be able to view mode of communication option phone if the phone is not available in the patient information', ({ given, and, when, then }) => {
    given('Admin get request from user (patient)', () => {
      defaultValidation()
    });
  ...
  test('EPIC_EPP-30_STORY_EPP-7529- Verify admin should be able to view "Share Username" button', ({ given, and, when, then }) => {
    given('Admin get request from user (patient)', () => {
      defaultValidation()
    });
    ...
    and(/^admin should be able to view "(.*)" button$/, (arg0) => {
      userViewShareUsernameBtn()
    });
  });

  test('EPIC_EPP-30_STORY_EPP-7529- Verify admin should be able to view "Cancel" button', ({ given, and, when, then }) => {
    given('Admin get request from user (patient)', () => {
      defaultValidation()
    });
  ...
  test('EPIC_EPP-30_STORY_EPP-7529- Verify admin should be able to view the success message ‘Username shared to <Patient name> successfully’ if ‘yes’ is selected during confirmation', ({ given, and, when, then }) => {
    given('Admin get request from user (patient)', () => {
      defaultValidation()
    });
    ...
    then(/^admin should be able to view the success message ‘Username shared to (.*) successfully’ if ‘yes’ is selected during confirmation$/, (arg0) => {
      adminSeeSuccessShareUsernameMsg()
    });
  });

  test('EPIC_EPP-30_STORY_EPP-7529- Verify The respective patient should receive the message with the username in the mode of communication selected by admin', ({ given, and, when, then }) => {
    given('Admin get request from user (patient)', () => {
      defaultValidation()
    });
  ...
  test('EPIC_EPP-30_STORY_EPP-7529- Verify The respective patient should not receive the message with username if No is selected during confirmation', ({ given, and, when, then }) => {
    given('Admin get request from user (patient)', () => {
      defaultValidation()
    });
