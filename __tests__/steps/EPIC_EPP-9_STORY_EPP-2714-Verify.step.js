...
defineFeature(feature, (test) => {
  ...
  const userSeeEmptyDocumentTable = () => {
    const emptyTable = container.getByText("There are no .");
    expect(emptyTable).toBeInTheDocument()
  }

  const userSeeTableAndDownloadBtn = () => {
    setTimeout(async () => {
      const tableDocument = await waitFor(() => container.getByTestId("table-documents"));
      expect(tableDocument).toBeInTheDocument()
      const downloadBtn = await waitFor(() => container.getByTestId("downloadPDFButton"));
