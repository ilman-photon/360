...
const feature = loadFeature(
  "./__tests__/features/Patient Portal/Sprint2/EPP-216.feature"
);
  ...
  test("EPIC_EPP-7_STORY_EPP-216 - Verify user should be able to login into the patient portal without username & password using the magic link received to my registered mail id.", ({
    ...
    then(`user should see One-time link page`, () => {
      ...
      confirmationFormProps.title = "One-time link"
        ...
        container = render(<ConfirmationForm
    ...
    and(`user should see "Choose a mode of communication where we should send you the magic link." text`, () => {
      const subtitle = container.getByText(/Choose a mode of communication where we/i);
      expect("Choose a mode of communication where we should send you the magic link.").toEqual(subtitle.textContent);
    });
    ...
    when(`user clicks on "Send magic link" button`, () => {
      const button = container.getByRole("button", { name: /Send one-time link/i });
