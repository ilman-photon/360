...
const feature = loadFeature(
  "./__tests__/feature/Patient Portal/Sprint9/EPP-7528.feature"
);
...
defineFeature(feature, (test) => {
  ...
  async function userClickSecurityQuestion() {
  ...
  async function userClickEmptySecurityQuestion() {
    const securityQuestionBtn = await waitFor(() => container.getAllByTestId("menu-more-view-security-questions"))
    const patientId = "8ad30e6a-db45-4147-b108-0d8a4c810782"
    mock
      .onGet(`/ecp/accountRecovery/viewSecurityQuestions/${patientId}`)
  ...
  async function userViewSecurityQuestions() {
  ...
  async function userNotYetSetupSecurityQuestions() {
    const securityQuestionBtn = await waitFor(() => container.getAllByTestId("menu-more-view-security-questions"))
    expect(securityQuestionBtn[0]).toBeInTheDocument()
    fireEvent.click(securityQuestionBtn[0])
    global.fetch = () => {
  ...
  async function userSeeNoSecurityQuestions() {
    const noSecurityQuestions = await waitFor(() => container.getByText(/User has not set-up security questions./i))
    expect(noSecurityQuestions).toBeInTheDocument()
  }

  test('EPIC_EPP-30_STORY_EPP-7528- Verify User should be able to view the security questions and answers set by the patient', ({ given, and, when, then }) => {
    ...
    when('User clicks on security question & answers CTA', () => {
      userClickSecurityQuestion()
    });

    then('User should be able to view the security questions and answers set by the patient', () => {
      userViewSecurityQuestions()
    });
  });

  test('EPIC_EPP-30_STORY_EPP-7528- Verify User should be able to view the message” No security question &Answers”', ({ given, and, when, then }) => {
    given('User has logged into the patient portal', () => {
      userIsLoggedIn();
    });
