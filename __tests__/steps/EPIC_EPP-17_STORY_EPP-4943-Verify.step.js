...
const feature = loadFeature(
  "./__tests__/feature/Patient Portal/Sprint6/EPP-4943.feature",
  {
...
defineFeature(feature, (test) => {
  ...
  test('EPIC_EPP-17_STORY_EPP-4943 - Verify the patient is able to navigate to prescription screen.', ({ given, when, and, then }) => {
  ...
  test('EPIC_EPP-17_STORY_EPP-4943 - Verify the default page appearence when prescription page is loaded.', ({ given, when, and, then }) => {
    given('patient launch the browser and enter the patient portal url', async () => {
      container = await renderLogin(container);
    });
    ...
    then(/^patient should be able to view the verbiage (.*) if there are no glasses prescriptions are available$/, (arg0) => {
      defaultValidation()
    });
  ...
  test('EPIC_EPP-17_STORY_EPP-4943- Verify the patient is able see the verbiage message in glasses section if there are no glasses prescriptions are available.', ({ given, when, and, then }) => {
    given('patient launch the browser and enter the patient portal url', async () => {
      container = await renderLogin(container);
    });
    ...
    when('patient navigates and click on glasses section', async() => {
      const button = container.getAllByTestId("menu-glasses")[0];
      fireEvent.click(button);
      await waitFor(()=> container.getAllByText("Glasses Prescription"))[0]
    });

    then(/^patient should be able to view the verbiage (.*) if there are no glasses prescriptions are available$/, (arg0) => {
      expect(container.getAllByText(/There are currently 0 prescriptions for glasses/i)[0]).toBeInTheDocument()
    });
  });

  test('EPIC_EPP-17_STORY_EPP-4943- Verify the patient is able see the glasses prescriptions details in glasses section if prescription details are available in Prescriptions page.', ({ given, when, and, then }) => {
    given('patient launch the browser and enter the patient portal url', async () => {
      container = await renderLogin(container);
    });
    ...
    when('patient navigates and click on glasses section', async () => {
      const button = container.getAllByTestId("menu-glasses")[1];
      fireEvent.click(button);
      await waitFor(()=> container.getAllByText("Glasses Prescription"))[0]
    });
    ...
    and(/^patient should see the (.*) with Doctor (.*)$/, (arg0, arg1) => {
      expect(container.getAllByText("indraku kumar")[1]).toBeInTheDocument()
    });
