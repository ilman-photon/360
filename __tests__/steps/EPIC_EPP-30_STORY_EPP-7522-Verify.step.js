...
const feature = loadFeature(
  "./__tests__/feature/Patient Portal/Sprint9/EPP-7522.feature"
);
...
defineFeature(feature, (test) => {
  ...
  async function userClickSendPasswordCTA() {
    const sendPasswordResetBtn = await waitFor(() => container.getAllByTestId("menu-more-send-password-reset"))
    fireEvent.click(sendPasswordResetBtn[0])
  }

  async function userViewCommunicationModeForm() {
    const inputCommunication = await waitFor(() => container.getAllByTestId("account-recovery-communication-mode-form"))
    expect(inputCommunication[0]).toBeInTheDocument()
  }

  async function userSeePhoneOption() {
    const phoneSelect = await waitFor(() => container.getByTestId("phone-test"))
    expect(phoneSelect).toBeInTheDocument()
  }

  async function useSeeConfirmSendPassword() {
    const confirmMsg = await waitFor(() => container.getByText("Are you sure you want to send password reset"))
    expect(confirmMsg).toBeInTheDocument()
    const submitBtn = await waitFor(() => container.getAllByText("Send Password"))
    act(() => {
      fireEvent.click(submitBtn)
    })
  }

  async function userSelectYesOption() {
    const yesBtn = await waitFor(() => container.getByText("Yes"))
    fireEvent.click(yesBtn)
  }

  async function userSelectNoOption() {
    const noBtn = await waitFor(() => container.getByText("No"))
    fireEvent.click(noBtn)
  }

  async function userSeeSuccessSendPasswordMsg() {
    const successMsg = await waitFor(() => container.getByText("Password reset link sent to patient successfully"))
    expect(successMsg).toBeInTheDocument()
  }

  test('EPIC_EPP-30_STORY_EPP-7522- Verify admin should be able to view the list of all locked accounts of patients along with details', ({ given, and, when, then }) => {
  ...
  test('EPIC_EPP-30_STORY_EPP-7522- Verify admin should not be able to view mode of communication option email if the email is not available in the patient information', ({  }) => {

  });

  test('EPIC_EPP-30_STORY_EPP-7522- Verify admin should not be able to view mode of communication option phone if the phone number is not available in the patient information', ({ given, and, when, then }) => {
    given('User has logged into the patient portal', () => {
      userIsLoggedIn();
    });
  ...
  test('EPIC_EPP-30_STORY_EPP-7522- Verify admin should be able to view the confirmation message “Are you sure to send reset password link’ along with ‘Yes’ and ‘No’ options when mode of communication is selected', ({ given, and, when, then }) => {
    given('User has logged into the patient portal', () => {
      userIsLoggedIn();
    });
    ...
    then('admin should be able to view the confirmation message “Are you sure to send reset password link’ along with ‘Yes’ and ‘No’ options when mode of communication is selected', () => {
      useSeeConfirmSendPassword()
    });
  });

  test('EPIC_EPP-30_STORY_EPP-7522- Verify admin should be able to view the success message ‘Password reset link sent to <Patient name> successfully’ if ‘yes’ is selected during confirmation', ({ given, and, when, then }) => {
    given('User has logged into the patient portal', () => {
      userIsLoggedIn();
    });
    ...
    then(/^admin should be able to view the success message ‘Password reset link sent to (.*) successfully’ if ‘yes’ is selected during confirmation$/, (arg0) => {
      userSeeSuccessSendPasswordMsg()
    });
  });

  test('EPIC_EPP-30_STORY_EPP-7522- Verify the respective patient should receive the message with the password reset link in the mode of communication selected by admin', ({ given, and, when, then }) => {
    given('User has logged into the patient portal', () => {
      userIsLoggedIn();
    });
  ...
  test('EPIC_EPP-30_STORY_EPP-7522- Verify the respective patient should not receive the message with the password reset link if No is selected during confirmation', ({ given, and, when, then }) => {
    given('User has logged into the patient portal', () => {
      userIsLoggedIn();
    });
