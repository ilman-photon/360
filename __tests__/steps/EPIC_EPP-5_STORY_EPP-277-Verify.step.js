...
defineFeature(feature, (test) => {
  ...
  test("EPIC_EPP-3_STORY_EPP-277 - Verify that  User should be prompted regarding session time out in multiple devices", ({
    given,
    when,
    and,
    then,
    ...
    and(
      "user navigates to the Patient Portal application in Device A",
      async () => {
    ...
    then(
      /^User should validate whether both are logged out after (\d+) mins$/,
      (arg0) => {
        defaultValidation();
      }
    );
