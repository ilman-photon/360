...
defineFeature(feature, (test) => {
  ...
  test('EPIC_EPP-8_STORY_EPP-945 - Verify that  User should be able to view my profile information/ demographic details in Desktop', ({ given, and, when, then }) => {
    ...
    then('user able to view "Personal information" section in left side and "Contact information" section in right side', async () => {
  ...
  test('EPIC_EPP-8_STORY_EPP-945 - Verify that  User should be able to view my profile information/ demographic details1', ({ given, and, when, then }) => {
    ...
    then('user able to view "Contact information" section', async () => {
        ...
        const contactButton = await waitFor(() => screen.getAllByText("Contact")[0]);
        fireEvent.click(contactButton);

        await waitFor(() => expect(screen.getAllByText("Phone Number")[0]).toBeInTheDocument());
        await waitFor(() => expect(screen.getAllByText("Email ID")[0]).toBeInTheDocument());
        await waitFor(() => expect(screen.getAllByText("Address")[0]).toBeInTheDocument());
        await waitFor(() => expect(screen.getAllByText("City")[0]).toBeInTheDocument());
        await waitFor(() => expect(screen.getAllByText("State")[0]).toBeInTheDocument());
        await waitFor(() => expect(screen.getAllByText("Zip")[0]).toBeInTheDocument());
        await waitFor(() => expect(screen.getAllByText("Prefered Mode(s) of communication")[0]).toBeInTheDocument());
    });
    ...
    then('user able to view the "Name", "Prefered Name", "Title", "Date of Birth", "Photo", "Age", "Gender", "SSN" and "State Issued ID" fields under "Personal information" section', async () => {
        ...
        await waitFor(() => expect(screen.getAllByText("Age")[0]).toBeInTheDocument());
        await waitFor(() => expect(screen.getAllByText("Gender")[0]).toBeInTheDocument());
        await waitFor(() => expect(screen.getAllByText("SSN")[0]).toBeInTheDocument());
        await waitFor(() => expect(screen.getAllByText("State Issued ID")[0]).toBeInTheDocument());
    });
  });
  ...
  test('EPIC_EPP-8_STORY_EPP-945 - Verify that  User should be able to view my profile information/ demographic details2', ({ given, and, when, then }) => {
    ...
    then('user able to view the "Phone Number", "Email ID", "Address", "City", "State", "Zip", "Prefered mode of communication" fields under "Contact information" section', async () => {
        await waitFor(() => screen.getAllByText("Contact")[0]);
        const contactButton = await waitFor(() => screen.getAllByText("Contact")[0]);
        fireEvent.click(contactButton);
