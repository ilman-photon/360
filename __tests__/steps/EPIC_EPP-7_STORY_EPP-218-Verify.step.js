...
const feature = loadFeature(
  "./__tests__/features/Patient Portal/Sprint2/EPP-218.feature"
);
  ...
  test("EPIC_EPP-7_STORY_EPP-218 - Verify the error message if user enter 3 times wrong/incorrect answer the security questions via \"Answer security questions\" mode", ({
    ...
    then("user should see \"Password Recovery Security Questions\" page", () => {
        const securityQuestions = [
            {Question: "Where did you go the first time you flew on a plane?", Answer: "America"},
            {Question: "Who is your all-time favorite movie character", Answer: "Peter"},
            {Question: "In what city or town did your parents meet?", Answer: "Berlin"}
        ]
        container = render(<PasswordSecurityQuestion
    ...
    and("user should view the questions fields", () => {
        const question1 = container.getByLabelText("Where did you go the first time you flew on a plane?");
        const question2 = container.getByLabelText("Who is your all-time favorite movie character");
        const question3 = container.getByLabelText("In what city or town did your parents meet?");
    ...
    when("user fills in wrong \"Question1Ans\" and \"Question2Ans\" for the security questions they set up 3 times", () => {
        const continueId = container.getByRole("button", { name: /continueButton/i });
        const question1 = container.getByLabelText("Where did you go the first time you flew on a plane?");
        const question2 = container.getByLabelText("Who is your all-time favorite movie character");
        const question3 = container.getByLabelText("In what city or town did your parents meet?");
    ...
    when("user should see error message \"Your account has been locked. Please contact Customer Support to unlock your account.\"", async () => {
        const continueId = container.getByRole("button", { name: /continueButton/i });
