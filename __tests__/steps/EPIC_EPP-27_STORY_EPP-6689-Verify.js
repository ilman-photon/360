...
import EducationMaterialsComponent from "../../src/components/organisms/EducationMaterials/educationMaterials";
...
import EducationMaterialsPage from "../../src/pages/patient/education-materials/index";
const useRouter = jest.spyOn(require("next/router"), "useRouter");

const feature = loadFeature(
  "./__tests__/feature/Patient Portal/Sprint8/EPP-6689.feature"
);
...
defineFeature(feature, (test) => {
    ...
    mock.onGet(`/ecp/patient/getPatientDocumentByCategory/98f9404b-6ea8-4732-b14f-9c1a168d8066/documents?pageSize=10&pageNo=0&sortBy=updated&sortOrder=dsc&search.query=((category=eq=EducationMaterials))`).reply(200, educationMaterials);
      ...
      container.rerender(
        <Provider store={store}>
          {EducationMaterialsPage.getLayout(<EducationMaterialsPage />)}
        </Provider>
      );
    });
    await waitFor(() => container.getByTestId("education-content-0"));
  };

  const renderEmptyComponent = () => {
    act(() => {
      container.rerender(
        <EducationMaterialsComponent eduMatData={[]} />
      )
    })
  }

  test('EPIC_EPP-27_STORY_EPP-6689 - Verify whether the user is able to navigate the Dashboard page', ({ given, when, and, then }) => {
    ...
    then('user must be able to view Image', () => {
      expect(container.getAllByTestId("educationMaterialsImg")[0]).toBeInTheDocument()
    });
  ...
  test('EPIC_EPP-27_STORY_EPP-6689 - Verify whether the User views the ‘Education Materials\' sub-menu under the ‘Documents’ menu', ({ given, when, and, then }) => {
    given('user launch the browser and enter the patient portal url', () => {
      window.matchMedia = createMatchMedia("1290px");
      launchBrowser();
    });
  ...
  test('EPIC_EPP-27_STORY_EPP-6689 - Verify whether the User is able to view the list of education materials', ({ given, when, and, then }) => {
    given('user launch the browser and enter the patient portal url', () => {
      window.matchMedia = createMatchMedia("1290px");
      launchBrowser();
    });
  ...
  test('EPIC_EPP-27_STORY_EPP-6689 - Verify whether the User is able to view Image, Name and Description', ({ given, when, and, then }) => {
    given('user launch the browser and enter the patient portal url', () => {
      window.matchMedia = createMatchMedia("1290px");
      launchBrowser();
    });
  ...
  test('EPIC_EPP-27_STORY_EPP-6689 - Verify whether the User is able to download the education material document and also Print the education material document', ({ given, when, and, then }) => {
    given('user launch the browser and enter the patient portal url', () => {
      window.matchMedia = createMatchMedia("1290px");
      launchBrowser();
    });
    ...
    then('user must be to view Print option', () => {
      const printBtn = container.getAllByTestId("printButton")[0];
  ...
  test('EPIC_EPP-27_STORY_EPP-6689 - Verify whether the User is able to view There is no educational material document when there is no education material documents for the user', ({ given, when, and, then }) => {
    given('user launch the browser and enter the patient portal url', () => {
      window.matchMedia = createMatchMedia("1290px");
      launchBrowser();
    });
