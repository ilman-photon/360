...
WHEN User selects the option to change the location
THEN User should get navigated to the screen to select the location as in https://eyecare-partners.atlassian.net/browse/EPP-2535 
And User should be able to change the location by searching locations using either City or State or Zipcode along with an option to detect their location (Locate me)
And User should be able to select a location based on the search
And System might have to remove the selection made for purpose of visit, insurance carrier, date of appointment and time slot of provider if the updated location does not support them and inform the user
